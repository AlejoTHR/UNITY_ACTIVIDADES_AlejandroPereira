...
public class Destroy_Radius : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Collider2D ObjDetected = Physics2D.OverlapCircle(transform.position, detection, DefaultRaycastLayers, 1f, 3f);
        
        Destroy(ObjDetected.gameObject);

    }

    public void ObjectsInRange(float range)
    {
        Collider2D ColliderInRange = Physics2D.OverlapCircle(transform.position, range);

        Destroy(ColliderInRange.gameObject);
    }
